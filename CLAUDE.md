...
- `worker/worker.js`, `worker/wrangler.toml` — the Cloudflare Worker used only by the static deployment (CORS proxy + serves the static build + the `/bosses` redirect), plus the `SnipeSession` Durable Object backing the Trade Sniper page's `/snipe/start`/`/snipe/poll`/`/snipe/stop` routes (see Trade Sniper section below). Irrelevant to running `boss.py` locally except that the Trade Sniper page renders but doesn't function locally (no DO relay outside the deployed Worker).
...
- **`PAGES`** (list of dicts: `slug`, `icon`, `menu_key`, `label_en`) — the single source of truth
  for the site-menu drawer. `render_sitemenu(current_slug)` generates every `<a class="sitemenu-
  link">` from this list, marking the current page active. **Adding a page = adding one entry
  here** — the menu updates itself everywhere, nothing to hand-edit per page.
...
- **`SHARED_JS_CHROME`** — `escAttr()`, theme toggle (`theme`/`applyTheme()` + its click wiring),
  ...
  (`showPopover`/`hidePopover`/`INFO_SEL`/the 4 delegated document listeners), and `lang`/`t(key)`/
  `applyStaticI18n()`. Concatenated **before** each page's own JS in the final `<script>` block —
...
- **To add a new page**: one `PAGES` entry, its own `<PAGE>_BODY`/`<PAGE>_JS` (+ `_EXTRA_CONTROLS`
  if it needs header controls) and `I18N` (seeded from the shared-key list above), a
  `render_<page>_page()` function following `render_bosses_page()`, one new `do_GET` branch, and
  one new `build_static.py` build call. Deliberately mechanical/explicit, not automatic — with only
  one real page today, generic "loop over all registered pages" machinery would be speculative.
...
- **`build_static.py` builds both pages**: `build/bosses/index.html` (via `render_page()`, the pricing-engine swap described above) and `build/snipe/index.html` (via `render_snipe_page()` — no pricing-engine swap needed, since the Trade Sniper page talks to `/snipe/*` on the same origin unchanged in both dev and prod; only `__LEAGUE_JSON__` gets substituted, same as `boss.py`'s own `main()` does locally). `sitemap.xml` lists both URLs.
