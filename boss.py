...
def build_index(league):
    ...
    # 11 poe.ninja calls plus poe.watch's heavy compact feed used to run
...
body{
  background:radial-gradient(1200px 600px at 50% -10%, #1a1622 0%, transparent 60%), var(--bg);
  color:var(--ink); font-family:"Spectral",Georgia,serif; font-size:15px;
...
header{border-bottom:1px solid var(--line);
  background:linear-gradient(180deg,#12101a,#0b0b0f);
...
.brand{font-family:"Cinzel",serif; letter-spacing:.14em; text-transform:uppercase}
...
button.sync{font-family:"Cinzel",serif; font-size:11px; letter-spacing:.18em;
...
.sortby-lbl{font-family:"Cinzel",serif; font-size:10px; letter-spacing:.1em; text-transform:uppercase;
...
.popover{display:none; position:fixed; z-index:50; max-width:280px;
  background:#1b1b26; border:1px solid var(--line); border-radius:4px; padding:10px 12px;
  box-shadow:0 14px 34px -14px rgba(0,0,0,.8); font-family:"Spectral",Georgia,serif;
...
.navtitle{display:block; font-family:"Cinzel",serif; font-size:11px; letter-spacing:.14em;
...
.navitem:hover{color:var(--ink); background:rgba(255,255,255,.03)}
...
.group-head{display:flex; align-items:baseline; gap:10px; padding-bottom:12px;
  border-bottom:1px solid var(--line)}
.group-head h3{margin:0; font-family:"Cinzel",serif; font-size:13px; letter-spacing:.18em;
...
.card h2{margin:0; font-family:"Cinzel",serif; font-size:14px; letter-spacing:.04em;
...
.quantctl, .timectl{display:flex; align-items:center; gap:6px; padding:8px 15px;
  border-bottom:1px solid var(--line); background:rgba(255,255,255,.015); flex-wrap:wrap}
.quantctl .qlbl, .timectl .qlbl{font-family:"Cinzel",serif; font-size:9px; letter-spacing:.14em;
...
.tier{font-family:"Cinzel",serif; font-size:9px; letter-spacing:.16em; text-transform:uppercase;
...
.profit{display:flex; justify-content:space-between; align-items:baseline; gap:8px;
  padding:11px 15px 6px; background:rgba(255,255,255,.015)}
.profit .lbl{font-family:"Cinzel",serif; font-size:10px; letter-spacing:.2em;
...
.pos{color:var(--ok)} .neg{color:#d98b6a} .na{color:var(--ink-dim)} .ink{color:var(--ink)}
.range{display:flex; justify-content:space-between; gap:10px; padding:0 15px 10px;
  background:rgba(255,255,255,.015); font-family:ui-monospace,monospace; font-size:11px}
...
.warn{color:#e0b050; font-size:11px; cursor:help}
.item .px .alt{display:block; color:#e0b050; font-size:10px; opacity:.9}
...
.err{color:#d98b6a; padding:16px 0}

footer{border-top:1px solid var(--line); margin-top:24px;
  background:linear-gradient(0deg,#12101a,#0b0b0f)}
...
<header>
  <div class="head">
    ...
    <div class="meta">
      ...
      <div class="chip"><span data-i18n="chip_price">Price</span> <b id="src">—</b></div>
      <div class="chip"><span data-i18n="chip_sync">Sync</span> <b id="ago">—</b> · <span data-i18n="chip_next">next</span> <b id="next">—</b></div>
    ...
    class H(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if path == "/":
                ...
                try:
                    body = json.dumps(build_payload(league)).encode("utf-8")
